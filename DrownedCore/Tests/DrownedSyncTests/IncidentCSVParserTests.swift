...
struct IncidentCSVParserTests {
    ...
    func multilineQuotedField() async throws -> Void {
        ...
        let first = try #require(
            incidents.first { incident in
                return incident.webID == "2014.MMP00001"
            })
        #expect(first.locationDescription.contains("\n"))

        let second = try #require(
            incidents.first { incident in
                return incident.webID == "2014.MMP00002"
            })
    ...
    func leadingBOM() async throws -> Void {
        let data = try #require(("\u{feff}" + Self.fixture).data(using: .utf8))
        let incidents = try await IncidentCSVParser().parse(data)
    ...
    func currentHDXExportParses() async throws -> Void {
    ...
    func liveEndpointDownloadParsesWhenPresent() async throws -> Void {
